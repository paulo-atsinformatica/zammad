...
QUnit.test('handles external data source attribute without any errors #5298', (assert) => {
  var { testCount, testName } = testSetup([{ name: 'ticket_allow_expert_conditions', value: true }])
  var testFormId = `form${testCount}`
  $('#forms').append(`<hr><h1>${testName} #${testCount}</h1><form id="${testFormId}"></form>`)
  var el = $(`#${testFormId}`)
      ...
      conditions: [
        {
          ...
          value: [
            {
              value: 78,
              label: 'Apple MacBook Pro 14 Inch Space Grey',
            },
            {
              value: 159,
              label: 'iPad Mini 2021 Starlight',
            },
          ],
        },
      ],
  ...
  assert.equal(el.find('.js-filterElement:nth-child(2) .js-attributeSelector select option:selected').text(), 'Product', 'product attribute selected')
  assert.equal(el.find('.js-filterElement:nth-child(2) .js-operator select option:selected').text(), 'is', 'is operator selected')
  assert.equal(el.find('.js-filterElement:nth-child(2) .js-value .token-label').eq(0).text(), 'Apple MacBook Pro 14 Inch Space Grey', 'Apple MacBook Pro 14 Inch Space Grey input value')
  assert.equal(el.find('.js-filterElement:nth-child(2) .js-value .token-label').eq(1).text(), 'iPad Mini 2021 Starlight', 'iPad Mini 2021 Starlight input value')

  el.find('.js-filterElement:nth-child(2) .js-value .js-remove').eq(1).trigger('click')

  assert.equal(el.find('.js-filterElement:nth-child(2) .js-value .token-label').text(), 'Apple MacBook Pro 14 Inch Space Grey', 'Apple MacBook Pro 14 Inch Space Grey input value')
