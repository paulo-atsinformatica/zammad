...
CodeMirror.defineMode("json", function(config, parserConfig) {
  ...
  return CodeMirror.overlayMode(CodeMirror.getMode(config, parserConfig.backdrop || "application/json"), jsonOverlay);
});
...
CodeMirror.registerHelper("lint", "json", function(text, options) {
  var found = [];
  if (!window.jsonlint) {
    if (window.console) {
  ...
  // Do not lint empty text if configured.
  if (options.skipEmpty && (text == undefined || text == null || text == '')) return found;
