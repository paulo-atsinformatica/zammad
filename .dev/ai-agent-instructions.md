...
## Modificações do Repositório Original

**IMPORTANTE**: Este é um fork customizado do Zammad com alterações de branding para ATS.

- **Documentação completa das modificações**: [`custom-branding-ats-modifications.md`](./custom-branding-ats-modifications.md)
- **Guia de sincronização com upstream**: [`sync-upstream-guide.md`](./sync-upstream-guide.md)
- Este repositório contém customizações visuais que substituem as cores padrão do Zammad (verde e azul) pela identidade visual ATS (vermelho escuro `#480404`).
- Principais alterações incluem:
  - Substituição de cores verdes e azuis por vermelho escuro (`#480404`) e vermelho claro (`#a40404`)
  - Customização de botões, ícones, navegação e elementos de interface
  - Ajustes para compatibilidade com tema claro e escuro
  - Alterações em arquivos SCSS, JavaScript (CoffeeScript), SVG e tokens CSS
- Além do branding, o fork adiciona dois sistemas próprios: **controle de pausas** e **tempo de atendimento por ticket**.

**Antes de fazer alterações**, consulte [`custom-branding-ats-modifications.md`](./custom-branding-ats-modifications.md) para entender todas as customizações aplicadas e evitar sobrescrever o branding ATS.
